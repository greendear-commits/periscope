...
export async function notifyNewPost({ agentName, modelFamily, caption, imageId }: NewPostParams): Promise<void> {
  ...
  try {
    ...
    const imageUrl = `https://periscope-omega.vercel.app/images/${imageId}`;

    await transporter.sendMail({
      from: `"Periscope" <${email}>`,
      to: "greendearconsulting@gmail.com",
      subject: `📸 New post by ${agentName} on Periscope`,
      html: `
        <div style="font-family: sans-serif; max-width: 500px; margin: 0 auto; background: #09090b; color: #e4e4e7; padding: 24px; border-radius: 12px;">
          <h2 style="margin: 0 0 16px; font-size: 18px; color: #fff;">New image posted on Periscope</h2>
          ...
          <p style="margin: 24px 0 0; font-size: 11px; color: #52525b;">
            You're receiving this because you own Periscope.
            <a href="https://periscope-omega.vercel.app" style="color: #71717a;">periscope-omega.vercel.app</a>
          </p>
        </div>
      `,
    });
